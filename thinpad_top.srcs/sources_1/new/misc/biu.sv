...
// ibus & dbus to bus, including addr, wdata, rdata, we, be (oe, ce will be computed)
wire [31:0] bus_addr =
    dbus_ce ? dbus_addr :
    ibus_ce ? ibus_addr :
    32'b0;
wire [31:0] bus_wdata =
    ...
    32'b0;
wire [31:0] bus_rdata =
    base_ram_ce ? base_ram_data :
    ext_ram_ce ? ext_ram_data :
    32'b0;
wire       bus_we = dbus_ce ? dbus_we : 1'b0;
wire       bus_oe = ~bus_we;
wire [3:0] bus_be = dbus_ce ? dbus_be : 4'b1111;
// bus to base_ram
assign base_ram_be_n   = ~bus_be;
assign base_ram_we_n   = ~bus_we;
assign base_ram_oe_n   = ~base_ram_we_n;  // if no write, use read
assign base_ram_ce_n   = ~
    (bus_addr >= `ADDR_BASERAM_START) && (bus_addr <= `ADDR_BASERAM_END);
wire base_ram_ce =
wire [3:0] ext_ram_be  = bus_be;
wire ext_ram_we        = bus_we;
wire ext_ram_oe        = ~ext_ram_we;
wire ext_ram_ce =
